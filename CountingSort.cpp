...
void countingSort(int tab[], int n) {
    //zmienna max i min przechowuje wartoœæ najwiêkszego i najmniejszego elementu tablicy
    int max =tab[0], min = tab[0];
    for (int i = 1; i < n; i++) {
        if (tab[i] > max) max = tab[i];
        if (tab[i] < min) min = tab[i];
    }
    //zmienna zakres okreœla rozmiar tablicy licz
    int zakres= max - min + 1;
    int licz[zakres] = {0};
    int wyjscie [n];

    //zliczanie wyst¹pieñ elementów
    for(int i = 0; i < n; i++) {
        licz[tab[i] - min]++;
    }
    ...
    //kopiowanie elementów z wyjœciowej tablicy do oryginalnej
    for(int i = 0; i < n; i++) {
        tab[i] = wyjscie[i];
    }
}
